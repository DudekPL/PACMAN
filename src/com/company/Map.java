    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < model.getSizex(); i++) {
            for (int j = 0; j < model.getSizey(); j++) {
                ...
                if (!model.field(i, j).canUp())
                    g2.fill(new Rectangle2D.Float(i * FIELD_SIZE, j * FIELD_SIZE, FIELD_SIZE, WALL_WIDTH));
                if (!model.field(i, j).canDown())
                    g2.fill(new Rectangle2D.Float(i * FIELD_SIZE, (j + 1) * FIELD_SIZE - WALL_WIDTH, FIELD_SIZE, WALL_WIDTH));
                if (!model.field(i, j).canLeft())
                    g2.fill(new Rectangle2D.Float(i * FIELD_SIZE, j * FIELD_SIZE, WALL_WIDTH, FIELD_SIZE));
                if (!model.field(i, j).canRight())
                    g2.fill(new Rectangle2D.Float((i + 1) * FIELD_SIZE - WALL_WIDTH, j * FIELD_SIZE, WALL_WIDTH, FIELD_SIZE));
                g2.setPaint(Color.WHITE);
                if (model.field(i, j).getInside() == Inside.DOT)
                    g2.fill(new Ellipse2D.Float((float) (i + 0.5) * FIELD_SIZE - r, (float) (j + 0.5) * FIELD_SIZE - r, 2 * r, 2 * r));
                if (model.field(i, j).getInside() == Inside.BIGDOT)
                    g2.fill(new Ellipse2D.Float((float) (i + 0.5) * FIELD_SIZE - 2 * r, (float) (j + 0.5) * FIELD_SIZE - 2 * r, 4 * r, 4 * r));
                g2.setPaint(Color.RED);
                if (model.field(i, j).getInside() == Inside.FRUIT)
                    g2.fill(new Ellipse2D.Float((float) (i + 0.5) * FIELD_SIZE - 2 * r, (float) (j + 0.5) * FIELD_SIZE - 2 * r, 4 * r, 4 * r));
            }
        }
    }
